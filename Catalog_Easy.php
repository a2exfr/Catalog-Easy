	...
	function getImageandInfo($title){
		...
		foreach ($file_sections as $key => $val) {
			...
			if (is_array($this->sect_options) && $this->sect_options['img_text'] == "yes"){
				if ($val['type'] <> "text"){
					unset($file_sections[$key]);
					$file_sections[$key] = ['type' => 'text', 'content' => '', 'attributes' => [],];
				}
			}
		}
		...
		return [$show, $short_info, $datafilter];
...
//	}
	...
	function ShowLayoutCarousel($items){
		...
		if ($this->ECheight <> ""){
			$style = 'style="height:' . $this->ECheight . 'px"';
		} else {
			$style = "";
		}
	...
	function getConfig(){
		...
		if (!isset($config)){
			...
			$opts = [
				'ShortInfo', 'AbbrevL', 'ShowTitlecar', 'ECthumb', 'ECthumbH', 'ECthumbW',
				'wap', 'catpages', 'nav_parent', 'nav_style', 'nav_buttons',
				'ImagesizeWpg', 'ImagesizeHpg', 'ImagesizeWsp', 'ImagesizeHsp', 'readmore_link',
				'readmore_text',
			];
			foreach ($opts as $opt) {
			...
			//deprecated options
			$opts = [
				'LinesPages', 'column3Pages', 'column2Pages', 'PGPages',
	...
	static function SectionTypes($section_types){
		$section_types['Catalog_easy_section'] = [];
	...
	static function SectionToContent($section_data){
		...
		$sect_options = [
			'catalog_layout' => $catalog_layout, 'source' => $source, 'cat_menu' => $cat_menu,
			'sourcepages' => $sourcepages, 'beh' => $beh, 'crop' => $crop, 'width' => $width,
			'height' => $height,
			'EC_thumb' => $EC_thumb, 'EC_id' => $EC_id, 'showimage' => $showimage,
			'shortinfo' => $shortinfo, 'abr' => $abr, 'ECrow' => $ECrow, 'ECheight' => $ECheight,
			'ShowTitlecar' => $ShowTitlecar,
			'ECPColumns' => $ECPColumns, 'ECPMinHeight' => $ECPMinHeight,
			'datafilter' => $datafilter, 'Showtitle' => $Showtitle,
...
//	get added section opts
	...
	static function NewSections($links){
		...
		foreach ($links as $key => $section_type_arr) {
			if ($section_type_arr[0] == 'Catalog_easy_section'){
				$links[$key] = [
					'Catalog_easy_section', $addonRelativeCode . '/img/catalog_sect.png',
				];
			}
	...
	static function PageRunScript($cmd){
		...
		if ($cmd == 'refresh_section'){
			...
			$sect_options = [
				...
				'showimage' => $showimage,
				'shortinfo' => $shortinfo, 'abr' => $abr, 'ECrow' => $ECrow,
				'ECheight' => $ECheight, 'ShowTitlecar' => $ShowTitlecar,
				'ECPColumns' => $ECPColumns, 'ECPMinHeight' => $ECPMinHeight,
				'datafilter' => $datafilter, 'Showtitle' => $Showtitle,
				'ItemW' => $ItemW, 'imagelinked' => $imagelinked, 'img_text' => $img_text,
			];
			...
			$page->ajaxReplace[] = ['refresh_replayFn', 'arg', $arg_value];
			return 'return';
		}
	...
	static function MenuPageOptions($title, $title_index, $level_arr = false, $layout_info){
		...
		echo common::Link('Admin_Menu', $img . $langmessage, 'cmd=menu_ec&index=' . urlencode($title), [
			'title' => $langmessage, 'data-cmd' => 'gpabox',
		]);
	}
	...
	function SaveData(){
		...
		if ($_REQUEST){
			if (array_key_exists("datafilter", $_REQUEST)){
				$_REQUEST["datafilter"] = array_diff($_REQUEST["datafilter"], ['']);
